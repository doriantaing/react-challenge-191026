  ...
  return(
    <div className="modal">
      <div className="modal-container">
        ...
        <form className="modal-form" onSubmit={item => sendForm(item, editMode)}>
          ...
          { allSubjects.map((subject, i) => {
            return(
              <GradeRadio
                subject={subject}
                key={i}
                last={i === allSubjects.length - 1}
                eventChange={inputOnChange}
                context={context}
                editMode={editMode}
              />
            )
          ...
          <div className={`modal-actions ${editMode && 'edit-active'}`}>
            { editMode && (
              <button className="btn btn-red" onClick={(e) => context.deleteStudent(e, studentID)}>Supprimer l'élève</button>
            )}
